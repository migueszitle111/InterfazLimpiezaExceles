...
class TabHoraWidget(QWidget):
    ...
    def update_y_axis(self, new_value):
        ...
        max_y = max(self.line_data + [new_value]) + 10  # Añadir un margen para la visibilidad
    ...
    def procesar_dataframe(self):
        ...
        self.dataframe['Hour'] = self.dataframe['Date Time'].dt.hour + self.dataframe['Date Time'].dt.minute / 60

        # Contar filas en cada rango horario
        conteo_por_rango = {}
        for rango, (hora_inicio, hora_fin) in rangos_horarios.items():
            conteo = self.dataframe[(self.dataframe['Hour'] >= hora_inicio) & (self.dataframe['Hour'] < hora_fin)].shape[0]
            conteo_por_rango[rango] = conteo
